...
export default async function handler(req, res) {
  ...
  try {
    const [
      ...
      // Units with computed occupancy fields
      query(`
        SELECT *,
      ...
      // All snapshots for trend (last 90 days)
      query(`
      ...
      // Tenant rates for discount/concession analysis
      query(`
        SELECT
          unit, unit_type, size_label, tenant_name,
          standard_rate, actual_rate, paid_rate, rate_variance,
          discount, discount_desc, days_as_tenant,
          ecri_flag, ecri_suggested, ecri_revenue_lift,
          moved_in
      ...
      // Aging for delinquency drag on economic occupancy
      query(`
        SELECT *
      ...
      // Rent roll for paid-thru / delinquency detail
      query(`
        SELECT unit, size_label, tenant_name, rent_rate, total_due, days_past_due, paid_thru
        FROM facility_pms_rent_roll
        WHERE facility_id = $1
          AND snapshot_date = (SELECT MAX(snapshot_date) FROM facility_pms_rent_roll WHERE facility_id = $1)
        ORDER BY days_past_due DESC
      `, [facilityId]),
    ])

    const units = unitsRes.rows || []
    const snapshots = (snapshotRes.rows || []).reverse() // chronological
    const tenantRates = tenantRatesRes.rows || []
    const agingRows = agingRes.rows || []
    const revenueHistory = revenueHistoryRes.rows || []
    const rentRoll = rentRollRes.rows || []

    // ── Facility-level physical vs economic ──
    ...
    const physicalOccPct = totalUnits > 0 ? Math.round(totalOccupied / totalUnits * 1000) / 10 : 0
    ...
    const economicOccPct = totalGrossPotential > 0
      ? Math.round(totalActualRevenue / totalGrossPotential * 1000) / 10
      : 0

    const occupancyGap = Math.round((physicalOccPct - economicOccPct) * 10) / 10

    // ── Gap decomposition: WHY is economic < physical? ──
    // 1. Vacancy drag (units not rented)
    const vacancyLoss = units.reduce((s, u) => {
      const vacant = parseInt(u.total_count || 0) - parseInt(u.occupied_count || 0)
      return s + (vacant * parseFloat(u.street_rate || 0))
    }, 0)
    const vacancyDragPct = totalGrossPotential > 0
      ? Math.round(vacancyLoss / totalGrossPotential * 1000) / 10 : 0

    // 2. Rate gap (tenants paying below street)
    const totalRateGapLoss = tenantRates.reduce((s, t) => {
      const variance = parseFloat(t.rate_variance || 0)
      return variance < 0 ? s + Math.abs(variance) : s
    }, 0)
    const rateGapDragPct = totalGrossPotential > 0
      ? Math.round(totalRateGapLoss / totalGrossPotential * 1000) / 10 : 0

    // 3. Discount/concession drag
    const totalDiscountDrag = tenantRates.reduce((s, t) => s + parseFloat(t.discount || 0), 0)
    const discountDragPct = totalGrossPotential > 0
      ? Math.round(totalDiscountDrag / totalGrossPotential * 1000) / 10 : 0

    // 4. Delinquency drag (money owed but not collected)
    const totalDelinquent = agingRows.reduce((s, r) => s + parseFloat(r.total || 0), 0)
    const delinquencyDragPct = totalGrossPotential > 0
      ? Math.round(totalDelinquent / totalGrossPotential * 1000) / 10 : 0

    // Delinquency by aging bucket
    const agingBuckets = {
      current: agingRows.reduce((s, r) => s + parseFloat(r.bucket_0_30 || 0), 0),
      days_31_60: agingRows.reduce((s, r) => s + parseFloat(r.bucket_31_60 || 0), 0),
      days_61_90: agingRows.reduce((s, r) => s + parseFloat(r.bucket_61_90 || 0), 0),
      days_91_120: agingRows.reduce((s, r) => s + parseFloat(r.bucket_91_120 || 0), 0),
      days_120_plus: agingRows.reduce((s, r) => s + parseFloat(r.bucket_120_plus || 0), 0),
    ...
    const unitOccupancy = units.map(u => {
      const total = parseInt(u.total_count || 0)
      const occupied = parseInt(u.occupied_count || 0)
      const vacant = total - occupied
      const streetRate = parseFloat(u.street_rate || 0)
      const avgActual = parseFloat(u.actual_avg_rate || 0)
      const physOcc = total > 0 ? Math.round(occupied / total * 1000) / 10 : 0
      const econOcc = parseFloat(u.economic_occ_pct || 0)
      const gap = Math.round((physOcc - econOcc) * 10) / 10
      const sqft = parseFloat(u.sqft || 0)
      const totalSqft = sqft * total
      const occupiedSqft = sqft * occupied
      const revenuePerSqft = occupiedSqft > 0 ? Math.round(avgActual / sqft * 100) / 100 : 0
      const potentialPerSqft = sqft > 0 ? Math.round(streetRate / sqft * 100) / 100 : 0
      ...
      const rateGap = streetRate > 0 && avgActual > 0 ? streetRate - avgActual : 0
      ...
      return {
        ...
        gross_potential: parseFloat(u.gross_potential || 0),
        actual_revenue: parseFloat(u.actual_revenue || 0),
    ...
    // ── Revenue-based occupancy trend from history ──
    const occupancyTrend = revenueHistory.map(r => {
      const revenue = parseFloat(r.revenue || 0)
      const moveIns = parseInt(r.move_ins || 0)
      const moveOuts = parseInt(r.move_outs || 0)
      return {
        year: r.year,
        month: r.month,
        month_short: (r.month || '').slice(0, 3),
        revenue,
        move_ins: moveIns,
        move_outs: moveOuts,
    ...
    // ── Snapshot-based occupancy trend ──
    const snapshotTrend = snapshots.map(s => ({
      date: s.snapshot_date,
      total_units: parseInt(s.total_units || 0),
      occupied_units: parseInt(s.occupied_units || 0),
      occupancy_pct: parseFloat(s.occupancy_pct || 0),
      total_sqft: parseInt(s.total_sqft || 0),
      occupied_sqft: parseInt(s.occupied_sqft || 0),
      sqft_occupancy_pct: parseInt(s.total_sqft || 0) > 0
        ? Math.round(parseInt(s.occupied_sqft || 0) / parseInt(s.total_sqft || 0) * 1000) / 10
        : 0,
      gross_potential: parseFloat(s.gross_potential || 0),
      actual_revenue: parseFloat(s.actual_revenue || 0),
      economic_occ_pct: parseFloat(s.gross_potential || 0) > 0
        ? Math.round(parseFloat(s.actual_revenue || 0) / parseFloat(s.gross_potential || 0) * 1000) / 10
        : 0,
    }))

    // ── Discount/concession analysis ──
    const discountedTenants = tenantRates
      .filter(t => parseFloat(t.discount || 0) > 0)
      .map(t => ({
        unit: t.unit,
        tenant: t.tenant_name,
        unit_type: t.unit_type,
        standard_rate: parseFloat(t.standard_rate || 0),
        actual_rate: parseFloat(t.actual_rate || 0),
        discount: parseFloat(t.discount || 0),
        discount_desc: t.discount_desc,
        days_as_tenant: parseInt(t.days_as_tenant || 0),
      }))

    // Tenants below street grouped by how far below
    const belowStreetBands = {
      slight: tenantRates.filter(t => { const v = parseFloat(t.rate_variance || 0); return v < 0 && v >= -10 }),
      moderate: tenantRates.filter(t => { const v = parseFloat(t.rate_variance || 0); return v < -10 && v >= -30 }),
      severe: tenantRates.filter(t => { const v = parseFloat(t.rate_variance || 0); return v < -30 }),
    }

    // ── Sqft-based occupancy (physical sqft vs economic sqft) ──
    const totalSqft = units.reduce((s, u) => s + (parseFloat(u.sqft || 0) * parseInt(u.total_count || 0)), 0)
    const occupiedSqft = units.reduce((s, u) => s + (parseFloat(u.sqft || 0) * parseInt(u.occupied_count || 0)), 0)
    const sqftOccPct = totalSqft > 0 ? Math.round(occupiedSqft / totalSqft * 1000) / 10 : 0

    const totalSqftRevenue = units.reduce((s, u) => {
      const occ = parseInt(u.occupied_count || 0)
      const avg = parseFloat(u.actual_avg_rate || 0)
      return s + (occ * avg)
    }, 0)
    const totalSqftPotential = units.reduce((s, u) => {
      const tot = parseInt(u.total_count || 0)
      const street = parseFloat(u.street_rate || 0)
      return s + (tot * street)
    }, 0)
    const sqftEconOccPct = totalSqftPotential > 0
      ? Math.round(totalSqftRevenue / totalSqftPotential * 1000) / 10 : 0

    // ── Rent roll delinquency detail ──
    const delinquentTenants = rentRoll
      .filter(r => parseInt(r.days_past_due || 0) > 0)
      .map(r => ({
        unit: r.unit,
        tenant: r.tenant_name,
        size: r.size_label,
        rent_rate: parseFloat(r.rent_rate || 0),
        total_due: parseFloat(r.total_due || 0),
        days_past_due: parseInt(r.days_past_due || 0),
        paid_thru: r.paid_thru,
        risk_level: parseInt(r.days_past_due || 0) > 90 ? 'critical'
          : parseInt(r.days_past_due || 0) > 60 ? 'high'
          : parseInt(r.days_past_due || 0) > 30 ? 'moderate' : 'low',
      }))
    ...
    if (belowStreetBands.severe.length > 0) {
      const severeRevLoss = belowStreetBands.severe.reduce((s, t) => s + Math.abs(parseFloat(t.rate_variance || 0)), 0)
      insights.push({
        type: 'critical',
    ...
    if (delinquentTenants.filter(t => t.risk_level === 'critical').length > 0) {
      const critCount = delinquentTenants.filter(t => t.risk_level === 'critical').length
      insights.push({
        type: 'critical',
        title: `${critCount} tenant${critCount > 1 ? 's' : ''} 90+ days delinquent`,
        detail: 'These units are occupied but generating zero revenue. Begin lien/auction process per state regulations.',
      })
    }

    // Size-category analysis (which sizes perform best?)
    ...
    return res.status(200).json({
      ...
      gap_decomposition: {
        ...
        total_drag: vacancyLoss + totalRateGapLoss + totalDiscountDrag + totalDelinquent,
      },
      ...
      below_street_bands: {
        slight: { count: belowStreetBands.slight.length, loss: belowStreetBands.slight.reduce((s, t) => s + Math.abs(parseFloat(t.rate_variance || 0)), 0) },
        moderate: { count: belowStreetBands.moderate.length, loss: belowStreetBands.moderate.reduce((s, t) => s + Math.abs(parseFloat(t.rate_variance || 0)), 0) },
        severe: { count: belowStreetBands.severe.length, loss: belowStreetBands.severe.reduce((s, t) => s + Math.abs(parseFloat(t.rate_variance || 0)), 0) },
      },
      delinquent_tenants: delinquentTenants,
      insights,
    })
  } catch (err) {
    console.error('Occupancy intelligence failed:', err.message)
