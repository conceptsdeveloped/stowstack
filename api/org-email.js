...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!orgId || !to || !templateKey) {
      return res.status(400).json({ error: 'orgId, to, and templateKey required' })
    }
          ...
          ${vars.channels ? `
            <div style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; padding: 16px; margin: 16px 0;">
              ...
              <ul style="margin: 0; padding-left: 20px; color: #15803d;">
                ${vars.channels.map(c => `<li style="margin-bottom: 4px;">${c}</li>`).join('')}
              </ul>
            </div>
          ` : ''}
    ...
    if (!process.env.RESEND_API_KEY) {
      return res.json({ success: true, preview: true, html: template.html, subject: template.subject })
    }
    ...
    console.error('Org email API error:', err)
    return res.status(500).json({ error: 'Internal server error' })
  }
}

function buildEmail(org, body) {
  const primaryColor = org.primary_color || '#16a34a'
  const brandName = org.white_label ? org.name : 'StowStack'
