...
export default async function handler(req, res) {
  ...
  try {
    const {
      name, email, phone, unitSize, timeline,
      facilityId, landingPageId, sessionId,
    ...
    // Upsert into partial_leads with full data
    await query(
    ...
    // Log activity if facility exists
    if (facilityId) {
      query(
        `INSERT INTO activity_log (type, facility_id, lead_name, detail)
         VALUES ('lead_captured', $1, $2, $3)`,
        [facilityId, name.trim(), `Lead from landing page: ${email.trim()}`]
    ...
    if (apiKey && facilityId) {
      // Get facility info for email context
      const facilities = await query('SELECT name, contact_email FROM facilities WHERE id = $1', [facilityId])
      const facilityName = facilities[0]?.name || 'Unknown Facility'
      ...
      // Also notify facility manager if they have an email
      if (facilities[0]?.contact_email) {
        recipients.push(facilities[0].contact_email)
      }
      ...
      }).catch(err => console.error('Lead notification email error:', err.message))
    }
