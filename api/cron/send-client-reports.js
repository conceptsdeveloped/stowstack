import { query } from '../_db.js'

/*
  Vercel Cron: Send Client Reports
  Schedule: Every Monday at 9am (0 9 * * 1)

  For weekly reporters: sends every Monday.
  For monthly reporters: sends on the first Monday of the month.
  Generates HTML report email with campaign KPIs, attribution breakdown, and trends.
*/

function esc(str) {
  if (!str) return ''
  return String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')
}

function formatCurrency(n) {
  return '$' + Number(n || 0).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
}

function trendArrow(current, previous, lowerIsBetter) {
  if (!previous || previous === 0) return ''
  const pctChange = Math.round(((current - previous) / previous) * 100)
  if (pctChange === 0) return '<span style="color:#94a3b8;">→ 0%</span>'
  const isGood = lowerIsBetter ? pctChange < 0 : pctChange > 0
  const color = isGood ? '#16a34a' : '#dc2626'
  const arrow = pctChange > 0 ? '↑' : '↓'
  return `<span style="color:${color};">${arrow} ${Math.abs(pctChange)}%</span>`
}

function renderReportHTML(data, clientName, facilityName, reportId) {
  const { current, previous, totals } = data
  const baseUrl = process.env.VERCEL_PROJECT_PRODUCTION_URL
    ? `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}`
    : 'https://stowstack.co'

  return `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
<body style="margin:0;padding:0;font-family:-apple-system,system-ui,sans-serif;background:#f8fafc;">
<div style="max-width:600px;margin:0 auto;padding:24px 16px;">
  <!-- Header -->
  <div style="text-align:center;margin-bottom:24px;">
    <div style="display:inline-block;width:36px;height:36px;background:linear-gradient(135deg,#10b981,#16a34a);border-radius:8px;margin-bottom:8px;"></div>
    <h1 style="margin:0;font-size:20px;color:#0f172a;">Your ${esc(facilityName)} Report</h1>
    <p style="margin:4px 0 0;font-size:13px;color:#94a3b8;">${data.periodLabel}</p>
  </div>

  <!-- KPI Cards -->
  <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
    <tr>
      <td style="width:50%;padding:4px;">
        <div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:16px;text-align:center;">
          <div style="font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;">Ad Spend</div>
          <div style="font-size:24px;font-weight:700;color:#0f172a;margin:4px 0;">${formatCurrency(current.spend)}</div>
          <div style="font-size:11px;">${trendArrow(current.spend, previous?.spend, false)}</div>
        </div>
      </td>
      <td style="width:50%;padding:4px;">
        <div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:16px;text-align:center;">
          <div style="font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;">Leads</div>
          <div style="font-size:24px;font-weight:700;color:#0f172a;margin:4px 0;">${current.leads}</div>
          <div style="font-size:11px;">${trendArrow(current.leads, previous?.leads, false)}</div>
        </div>
      </td>
    </tr>
    <tr>
      <td style="width:50%;padding:4px;">
        <div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:16px;text-align:center;">
          <div style="font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;">Move-Ins</div>
          <div style="font-size:24px;font-weight:700;color:#16a34a;margin:4px 0;">${current.moveIns}</div>
          <div style="font-size:11px;">${trendArrow(current.moveIns, previous?.moveIns, false)}</div>
        </div>
      </td>
      <td style="width:50%;padding:4px;">
        <div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:16px;text-align:center;">
          <div style="font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;">Cost / Move-In</div>
          <div style="font-size:24px;font-weight:700;color:#0f172a;margin:4px 0;">${current.moveIns > 0 ? formatCurrency(current.spend / current.moveIns) : '—'}</div>
          <div style="font-size:11px;">${current.moveIns > 0 && previous?.moveIns > 0 ? trendArrow(current.spend / current.moveIns, previous.spend / previous.moveIns, true) : ''}</div>
        </div>
      </td>
    </tr>
  </table>

  <!-- ROI Summary -->
  <div style="background:linear-gradient(135deg,#020617,#0f172a);border-radius:12px;padding:20px;text-align:center;margin-bottom:20px;">
    <div style="font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;">Estimated ROI</div>
    <div style="font-size:32px;font-weight:700;color:#10b981;margin:4px 0;">${current.roas > 0 ? current.roas.toFixed(1) + 'x' : '—'}</div>
    <div style="font-size:12px;color:#64748b;">Based on ${current.moveIns} move-ins × $110/mo avg rent × 12 months</div>
  </div>

  <!-- Attribution Breakdown -->
  ${current.leads > 0 ? `
  <div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:16px;margin-bottom:20px;">
    <h3 style="margin:0 0 12px;font-size:14px;color:#0f172a;">Lead Sources</h3>
    <table style="width:100%;border-collapse:collapse;">
      <tr style="border-bottom:1px solid #f1f5f9;">
        <td style="padding:8px 0;font-size:13px;color:#64748b;">Digital (tracked)</td>
        <td style="padding:8px 0;font-size:13px;font-weight:600;text-align:right;">${current.leads}</td>
      </tr>
      ${current.calls > 0 ? `<tr style="border-bottom:1px solid #f1f5f9;">
        <td style="padding:8px 0;font-size:13px;color:#64748b;">Phone calls (tracked)</td>
        <td style="padding:8px 0;font-size:13px;font-weight:600;text-align:right;">${current.calls}</td>
      </tr>` : ''}
      ${current.walkins > 0 ? `<tr style="border-bottom:1px solid #f1f5f9;">
        <td style="padding:8px 0;font-size:13px;color:#64748b;">Walk-ins (reported)</td>
        <td style="padding:8px 0;font-size:13px;font-weight:600;text-align:right;">${current.walkins}</td>
      </tr>` : ''}
    </table>
  </div>` : ''}

  <!-- CTA -->
  <div style="text-align:center;margin-bottom:24px;">
    <a href="${baseUrl}/portal" style="display:inline-block;padding:14px 32px;background:#16a34a;color:white;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px;">View Full Dashboard</a>
  </div>

  <!-- Footer -->
  <div style="text-align:center;font-size:11px;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:16px;">
    <p style="margin:0;">StowStack by StorageAds.com</p>
    <p style="margin:4px 0 0;">This report tracks the digital path from ad click to move-in. Some move-ins may be influenced by ads but complete offline — your full dashboard shows the complete picture.</p>
  </div>
</div>
<!-- Open tracking -->
<img src="${baseUrl}/api/report-open?id=${reportId}" width="1" height="1" style="display:none;" alt="" />
</body>
</html>`
}

export default async function handler(req, res) {
  const cronSecret = (process.env.CRON_SECRET || '').trim()
  if (cronSecret && req.headers.authorization !== `Bearer ${cronSecret}`) {
    return res.status(401).json({ error: 'Unauthorized' })
  }

  const results = { processed: 0, sent: 0, skipped: 0, errors: [] }

  try {
    const now = new Date()
    const dayOfMonth = now.getDate()
    const isFirstWeek = dayOfMonth <= 7

    // Get all report-enabled clients
    const clients = await query(
      `SELECT c.*, f.id AS fac_id, f.name AS fac_name
       FROM clients c
       JOIN facilities f ON c.facility_id = f.id
       WHERE c.report_enabled = true`
    )

    for (const client of clients) {
      try {
        results.processed++

        // Weekly: always send. Monthly: only on first week of month
        if (client.report_frequency === 'monthly' && !isFirstWeek) {
          results.skipped++
          continue
        }

        const isWeekly = client.report_frequency === 'weekly'
        const periodEnd = new Date(now)
        periodEnd.setDate(periodEnd.getDate() - 1) // yesterday
        const periodStart = new Date(periodEnd)
        if (isWeekly) {
          periodStart.setDate(periodStart.getDate() - 6) // last 7 days
        } else {
          periodStart.setMonth(periodStart.getMonth() - 1) // last month
          periodStart.setDate(1)
        }

        const periodStartStr = periodStart.toISOString().slice(0, 10)
        const periodEndStr = periodEnd.toISOString().slice(0, 10)

        // Check if already sent for this period
        const existing = await query(
          `SELECT id FROM client_reports WHERE client_id = $1 AND report_type = $2 AND period_start = $3`,
          [client.id, isWeekly ? 'weekly' : 'monthly', periodStartStr]
        )
        if (existing.length > 0) {
          results.skipped++
          continue
        }

        // Get campaign data for the period
        const campaigns = await query(
          `SELECT * FROM client_campaigns WHERE client_id = $1 ORDER BY month ASC`,
          [client.id]
        )

        if (campaigns.length === 0) {
          results.skipped++
          continue
        }

        const current = campaigns[campaigns.length - 1]
        const previous = campaigns.length >= 2 ? campaigns[campaigns.length - 2] : null

        // Get call count
        let callCount = 0
        try {
          const calls = await query(
            `SELECT COUNT(*)::int AS cnt FROM call_logs WHERE facility_id = $1 AND started_at >= $2 AND started_at <= $3`,
            [client.fac_id, periodStartStr, periodEndStr]
          )
          callCount = calls[0]?.cnt || 0
        } catch { /* no call tracking */ }

        // Get walk-in count
        let walkinCount = 0
        try {
          const walkins = await query(
            `SELECT COUNT(*)::int AS cnt FROM walkin_attributions WHERE facility_id = $1 AND created_at >= $2 AND created_at <= $3`,
            [client.fac_id, periodStartStr, periodEndStr]
          )
          walkinCount = walkins[0]?.cnt || 0
        } catch { /* no walkin data */ }

        const totals = campaigns.reduce((acc, c) => ({
          spend: acc.spend + Number(c.spend || 0),
          leads: acc.leads + Number(c.leads || 0),
          moveIns: acc.moveIns + Number(c.move_ins || 0),
        }), { spend: 0, leads: 0, moveIns: 0 })

        const reportData = {
          periodLabel: isWeekly
            ? `Week of ${periodStart.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} — ${periodEnd.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`
            : periodStart.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
          current: {
            spend: Number(current.spend || 0),
            leads: Number(current.leads || 0),
            moveIns: Number(current.move_ins || 0),
            cpl: Number(current.cpl || 0),
            roas: Number(current.roas || 0),
            calls: callCount,
            walkins: walkinCount,
          },
          previous: previous ? {
            spend: Number(previous.spend || 0),
            leads: Number(previous.leads || 0),
            moveIns: Number(previous.move_ins || 0),
            cpl: Number(previous.cpl || 0),
            roas: Number(previous.roas || 0),
          } : null,
          totals,
        }

        // Generate placeholder ID for the tracking pixel
        const reportRow = await query(
          `INSERT INTO client_reports (client_id, facility_id, report_type, period_start, period_end, report_html, report_data, status)
           VALUES ($1, $2, $3, $4, $5, '', $6, 'generated')
           RETURNING id`,
          [client.id, client.fac_id, isWeekly ? 'weekly' : 'monthly', periodStartStr, periodEndStr, JSON.stringify(reportData)]
        )
        const reportId = reportRow[0].id

        // Render HTML with the report ID for tracking
        const html = renderReportHTML(reportData, client.name, client.fac_name || client.facility_name, reportId)

        // Update with rendered HTML
        await query('UPDATE client_reports SET report_html = $1 WHERE id = $2', [html, reportId])

        // Send via Resend
        const apiKey = process.env.RESEND_API_KEY
        if (apiKey) {
          const emailRes = await fetch('https://api.resend.com/emails', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${apiKey}`,
            },
            body: JSON.stringify({
              from: 'StowStack <reports@stowstack.co>',
              to: client.email,
              subject: `${client.fac_name || client.facility_name} — ${isWeekly ? 'Weekly' : 'Monthly'} Performance Report`,
              html,
            }),
          })

          if (emailRes.ok) {
            await query('UPDATE client_reports SET sent_at = NOW(), status = $1 WHERE id = $2', ['sent', reportId])
            results.sent++
          } else {
            const errText = await emailRes.text()
            await query('UPDATE client_reports SET status = $1 WHERE id = $2', ['failed', reportId])
            results.errors.push(`Email failed for ${client.email}: ${errText}`)
          }
        }

        // Log activity
        query(
          `INSERT INTO activity_log (type, facility_id, facility_name, detail)
           VALUES ('report_sent', $1, $2, $3)`,
          [client.fac_id, client.fac_name, `${isWeekly ? 'Weekly' : 'Monthly'} report sent to ${client.email}`]
        ).catch(() => {})

      } catch (err) {
        results.errors.push(`${client.email}: ${err.message}`)
      }
    }

    return res.json({ success: true, ...results })
  } catch (err) {
    console.error('send-client-reports error:', err)
    return res.status(500).json({ error: err.message, ...results })
  }
}
